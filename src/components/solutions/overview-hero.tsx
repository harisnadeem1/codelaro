	...
	return (
		<div className="relative mx-auto mt-14 w-full max-w-md lg:mt-0 lg:max-w-none">
			...
			<div className="relative min-h-[420px] sm:min-h-[460px] lg:min-h-[520px]">
				...
				>
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-2">
							...
							<p className="text-[13px] font-semibold text-navy">Outcome-led</p>
						</div>
					...
					<div className="mt-4 space-y-2.5">
						{[
							{ label: 'Validate & ship', w: 'w-3/4' },
							{ label: 'Automate the manual', w: 'w-full' },
							{ label: 'Modernize legacy', w: 'w-5/6' },
							{ label: 'Put AI to work', w: 'w-2/3' },
						].map((row, i) => (
							<div key={row.label} className="flex items-center gap-2.5">
								<span className="font-mono text-[11px] font-semibold text-brand/70">
									0{i + 1}
								</span>
								<span className={`h-2 rounded-full bg-gradient-to-r from-brand/30 to-brand/70 ${row.w}`} />
							</div>
						))}
					</div>
				</div>
				...
				>
					<p className="text-[12px] font-semibold text-navy">Business result, not features</p>
					<p className="mt-2 text-[11px] leading-relaxed text-slate-400">
						Every solution is measured against a real outcome.
					</p>
				</div>
					...
					<div className="lg:col-span-7">
						...
						<h1 className="mt-6 font-display text-[2.1rem] font-bold leading-[1.08] tracking-tight text-navy sm:text-5xl lg:text-[3.75rem] lg:leading-[1.05]">
							<span className="rise-in block" style={{ animationDelay: '0.15s' }}>
								Built around the{' '}
								<span className="text-brand">outcome</span>,
							</span>
						...
						>
							...
							challenges. Each solution below targets a specific outcome — so the
							software we ship moves a measurable result, not just a feature checklist.
						</p>
